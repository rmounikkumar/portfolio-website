    ...
    <AnimatePresence>
      {show && (
        <motion.nav
          ...
          className="fixed top-0 left-0 right-0 z-50 flex justify-center py-5 px-6"
        >
          <div className="flex items-center gap-1 bg-[#050505]/80 backdrop-blur-xl border border-white/8 rounded-full px-2 py-2">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
